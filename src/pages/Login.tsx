...
const Login = () => {
  ...
  const handleCredentialLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Verificar senha (CPF como bigint)
      const senhaDigitada = senha.replace(/\D/g, ''); // Remove não-dígitos
      ...
      // Primeiro acesso - mostrar tela de vinculação
      setPendingUser(usuario);
      setShowLinkAccount(true);
      toast.success(`Bem-vindo, ${usuario.nome}! Vincule sua conta Google para continuar.`);
    } catch (error: any) {
  ...
  const handleGoogleLink = async () => {
    ...
    try {
      // Salvar ID do usuário para vincular após OAuth
  ...
  // Tela de vinculação de conta
  if (showLinkAccount && pendingUser) {
    return (
        ...
        <Card className="w-full max-w-md border-border shadow-lg">
          ...
          <CardContent className="space-y-4 px-4 sm:px-6">
            <div className="bg-muted/50 rounded-lg p-4 text-sm text-muted-foreground">
              <p className="mb-2">
                Este é seu primeiro acesso. Para maior segurança, vincule sua conta a um e-mail Google.
              </p>
              <p>
                Após vincular, você poderá acessar o sistema usando apenas o botão "Entrar com Google".
              </p>
            </div>
            ...
            >
              ...
              {isGoogleLoading ? 'Vinculando...' : 'Vincular com Google'}
            </Button>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-3 sm:p-4">
      <Card className="w-full max-w-md border-border shadow-lg">
        ...
        <CardContent className="space-y-4 px-4 sm:px-6">
          {/* Google Login Button - para usuários já vinculados */}
          <Button
            type="button"
            variant="outline"
            className="w-full flex items-center justify-center gap-2 h-10 sm:h-11 text-sm sm:text-base"
