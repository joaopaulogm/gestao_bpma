...
function applyFilters(rows: RadioRow[], filters: RadioFilters): RadioRow[] {
  ...
  return rows.filter((row) => {
    ...
    if (filters.equipe && String(row.data['Equipe'] ?? '').trim() !== filters.equipe) return false;
    if (filters.desfecho && String(row.data['Desfecho'] ?? '').trim() !== filters.desfecho) return false;
    return true;
  });
}
...
const RadioOperador: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [resResgates, resCrimes] = await Promise.all([
        (supabase as any).from('fat_controle_ocorrencias_resgate_2026').select(RESGATE_SELECT).order('data', { ascending: false, nullsFirst: false }).order('created_at', { ascending: false }),
        (supabase as any).from('fat_controle_ocorrencias_crime_ambientais_2026').select(RESGATE_SELECT).order('data', { ascending: false, nullsFirst: false }).order('created_at', { ascending: false }),
      ]);
      ...
      const lastCreated = [...resgatesFat, ...crimesFat].map((r) => r.created_at).filter(Boolean).sort().pop() ?? null;
  ...
  const handleSync = useCallback(async () => {
    setSyncing(true);
    try { await fetchData(); toast.success('Dados atualizados'); } catch {} finally { setSyncing(false); }
  }, [fetchData]);
  ...
  const resgatesDataRows = useMemo(() => sortByDateDesc(bySheet.get(SHEET_RESGATES) ?? []), [bySheet]);
  const crimesDataRows = useMemo(() => sortByDateDesc(bySheet.get(SHEET_CRIMES) ?? []), [bySheet]);
  const resgatesFiltered = useMemo(() => applyFilters(resgatesDataRows, filters), [resgatesDataRows, filters]);
  const crimesFiltered = useMemo(() => applyFilters(crimesDataRows, filters), [crimesDataRows, filters]);

  const { uniqueYears, uniqueMonths, uniqueDays, uniqueEquipes, uniqueDesfechos } = useMemo(() => {
    const all = [...resgatesDataRows, ...crimesDataRows];
    const years = new Set<number>(), months = new Set<number>(), days = new Set<number>();
    const equipes = new Set<string>(), desfechos = new Set<string>();
    for (const row of all) {
      const parts = parseDateParts(row.data['Data']);
      if (parts) { years.add(parts.year); months.add(parts.month); days.add(parts.day); }
      const eq = row.data['Equipe']; if (eq != null && String(eq).trim()) equipes.add(String(eq).trim());
      const df = row.data['Desfecho']; if (df != null && String(df).trim()) desfechos.add(String(df).trim());
    }
    return {
  ...
  const handleEditFromTable = useCallback((_row: RadioRow, _sheetName: string) => {
  ...
  const handleExport = useCallback((tabOverride?: 'resgates' | 'crimes') => {
    ...
    const body = list.map((row) => cols.map((c) => String(row.data[c.key] ?? '').replace(/;/g, ',')).join(';')).join('\n');
    const csv = '\uFEFF' + header + '\n' + body;
      ...
      <div className="px-6 py-4">
        {loading && !hasData ? (
          ...
          <div className="text-center py-24">
            ...
            <p className="text-lg font-medium text-foreground">Nenhum dado carregado</p>
            <p className="text-sm text-muted-foreground mt-1">Use o botão Atualizar para recarregar.</p>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <div className="flex items-center justify-between border-b border-border/60 pb-3 mb-4">
              <TabsList className="h-9 bg-muted/50 rounded-md p-0.5 gap-0.5">
                <TabsTrigger
                  value={SHEET_RESGATES}
                  className="data-[state=active]:bg-background data-[state=active]:shadow-sm rounded-md px-3 h-8 text-sm"
                >
                  <PawPrint className="h-4 w-4 mr-1.5" />
                  Resgate de Fauna
                  <Badge variant="secondary" className="ml-2 h-5 px-1.5 text-[10px] rounded-md bg-primary/10 text-primary">
              ...
              {lastSync && (
                <span className="text-xs text-muted-foreground">
                  Atualizado: {format(new Date(lastSync), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}
                </span>
              )}
            </div>
            ...
            <TabsContent value={SHEET_RESGATES} className="mt-2">
              <p className="text-sm text-muted-foreground mb-2">
                <span className="font-medium text-foreground">{resgatesFiltered.length}</span> ocorrências
              </p>
              <OcorrenciasTable
            ...
            <TabsContent value={SHEET_CRIMES} className="mt-2">
              <p className="text-sm text-muted-foreground mb-2">
                <span className="font-medium text-foreground">{crimesFiltered.length}</span> ocorrências
              </p>
              <OcorrenciasTable
                data={crimesFiltered}
